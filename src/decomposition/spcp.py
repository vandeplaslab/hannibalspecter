...
class spcp:
    """
    Perform stable principal component pursuit (see [1, 2]):
        a = b + c + d
    where
        a = input array
        b = low-rank array (u*s*vt)
        c = sparse error array
        d = residual error array (in Frobenius sense)
    ...
    def _theta_search(self, a: array) -> None:
